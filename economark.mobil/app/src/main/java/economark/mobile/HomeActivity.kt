...
class HomeActivity : AppCompatActivity() {
    ...
    private fun generarReporteBajoStock() {
        ...
        lifecycleScope.launch {
            try {
                // Simulación temporal (remplazar por tu lógica/API)
                val datos = listOf(
                ...
                generarPDF(datos)
                Toast.makeText(this@HomeActivity, "Reporte generado", Toast.LENGTH_SHORT).show()
                reportGeneratedLayout.visibility = View.VISIBLE

            } catch (e: Exception) {
                Toast.makeText(this@HomeActivity, "Error al generar reporte", Toast.LENGTH_SHORT).show()
            } finally {
